        ...
        private ValueMember ApplyDefaultBehaviour(bool isEnum, ProtoMemberAttribute normalizedAttribute)
        {
            ...
            if (model.UseImplicitZeroDefaults)
            {
                switch (Helpers.GetTypeCode(effectiveType))
                {
                    ...
                    //case ProtoTypeCode.SmartInt: defaultValue = (SmartInt)0; break;
            ...
            { // key-value-pair etc
                ...
                if(ResolveTupleConstructor(type, out mapping) != null)
                {
                    ...
                    for(int i = 0 ; i < mapping.Length ; i++)
                    {
                        ...
                        var sb = NewLine(builder, indent + 1).Append("optional ").Append(model.GetSchemaTypeName(effectiveType, DataFormat.Default, false, false, ref requiresBclImport).Replace('.', '_'))
                            .Append(' ').Append(mapping[i].Name).Append(" = ").Append(i + 1);

                        if (AppendNullable(effectiveType))
                        {
                            sb.Append("[(gogoproto.nullable) = false]");
                        }
            ...
            } else
            {
                ...
                foreach (ValueMember member in fieldsArr)
                {
                    ...
                    if(member.DefaultValue != null && member.IsRequired == false)
                    {
                        ...
                        {
                            builder.Append(" [default = \"").Append(member.DefaultValue).Append("\", (gogoproto.nullable) = false]");
                        }
                        else if(member.DefaultValue is bool)
                        {   // need to be lower case (issue 304)
                            builder.Append((bool)member.DefaultValue ? " [default = true, (gogoproto.nullable) = false]" : " [default = false, (gogoproto.nullable) = false]");
                        }
                        else
                        {
                            builder.Append(" [default = ").Append(member.DefaultValue).Append(", (gogoproto.nullable) = false]");
                    ...
                    else if (member.ItemType == null && AppendNullable(member.MemberType) )
                    {
                        builder.Append("[(gogoproto.nullable) = false]");
                    }
